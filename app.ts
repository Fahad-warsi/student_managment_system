...
class Student {
  ...
  pay_fees(fees_amount: number) {
    this.balance -= +fees_amount;
    console.log(
      `${fees_amount} has paid the fees successfully for ${this.name}. `
    );
  }
...
class Student_manager {
  ...
  add_student(name: string) {
    ...
    console.log(
      `Student : ${name} has successfully added. Student ID : ${student.id} `
    );
  }

  //method to enroll the courses
  enroll_courses(studnet_id: number, course: string) {
    let student = this.students.find((std) => std.id === studnet_id);
    if (student) {
      student.enroll_courses(course);
      console.log(
        `Student Name : ${student.name} enroll in ${course} successfully.`
      );
    }
  }
  ...
  pay_tution_fees(student_id: number, Amount: number) {
    let student = this.find_student(student_id);
    if (student) {
  ...
  while (true) {
    ...
    switch (choice.choice) {
      ...
      case "Enroll courses":
        ...
        student_manager.enroll_courses(
          course_input.std_id,
      ...
      case "Pay Fees":
        ...
        student_manager.pay_tution_fees(
          PayFees_input.std_id,
          PayFees_input.Amount
        );
        break;
      case "Show Status":
